...
h5files <- h5files[!(h5files$entity.name == "C8581ANXX_6_RNASeq_Kallisto_gencodev24_quants.h5" | 
                       h5files$entity.name =="C8581ANXX_3_RNASeq_Kallisto_gencodev24_quants.h5"),]

h5s=lapply(h5files[,2],function(x) synGet(x))
snames=lapply(h5s,function(x) x@annotations$sampleIdentifier)
sgens=lapply(h5s,function(x) x@annotations$nf1Genotype)
...
so<-sleuth_prep(df,~ Genotype, num_cores = 1) #failed here
so<-sleuth_fit(so)
