# modified from
# https://github.com/sgosline/pnfCellLines/blob/4009ddc118eb537dd60855996d39796ed448d713/bin/clusterCNVBySample.R
...
ann.vals<-synTableQuery('SELECT "Sample Name","Sample ID","Sample Genotype" FROM syn8397154')@values
...
if(!exists("geneInfo")){
  xref<-read.table('../../../../data/ucsc_kgXref_hg19.txt',header=F,as.is=T,sep='\t',quote='"')[,c(1,5)]
  genepos<-read.table('../../../../data/ucsc_knownGene_hg19.txt',header=F,as.is=T,sep='\t',quote='"')
...
plotClusteredSegmentData<-function(segdat,byval='gene',metric='median',topGenes=100,prefix=''){
  ...
  dev.off()
  colnames(tm) <- as.character(clnames[colnames(tm)])
  ...
  png(fname)
  if(byval=='gene')
    pheatmap(tm,annotation_row=data.frame(NF1Corr=nfcor, row.names = rownames(tm)),
             annotation_col=data.frame(Genotype=genotype,row.names = colnames(tm)),
             cellwidth=10,cellheight=10,file=fname)
  else
    pheatmap(tm,annotation_col=data.frame(Genotype=genotype,row.names = colnames(tm)),
             cellwidth=10,cellheight=10,file=fname)
